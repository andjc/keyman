...
lang = "Keyman"
log = logging.getLogger("codeintel.keyman")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "Keyman-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
        "any",
...
# lexers. You must define a Lexer class for lang Keyman. If Komodo's
# scintilla lexer for Keyman is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class KeymanLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for Keyman is one of Komodo's existing C++ lexers
...
#   class KeymanLexer(Lexer):
#       lang = lang
#       def __init__(self):
#           self._properties = SilverCity.PropertySet()
#           self._lexer = SilverCity.find_lexer_module_by_id(ScintillaConstants.SCLEX_KEYMAN)
#           self._keyword_lists = [
...
# examples of usage.
class KeymanLangIntel(LangIntel):
    lang = lang
...
# Keyman content and returns a description of the code in that file.
# See "cile_keyman.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If Keyman is
...
class KeymanCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_keyman
        return cile_keyman.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=KeymanLexer(),
        buf_class=KeymanBuffer,
        langintel_class=KeymanLangIntel,
        import_handler_class=None,
        cile_driver_class=KeymanCILEDriver,

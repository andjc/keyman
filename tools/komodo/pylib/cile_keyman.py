...
"""A Code Intelligence Language Engine for the Keyman language.
...
Module Usage:
    from cile_keyman import scan
    mtime = os.stat("bar.keyman")[stat.ST_MTIME]
    content = open("bar.keyman", "r").read()
    scan(content, "bar.keyman", mtime=mtime)
"""
...
class KeymanCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.keyman")
...
def scan_buf(buf, mtime=None, lang="Keyman"):
    """Scan the given KeymanBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {KeymanBuffer} is the Keyman buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the Keyman CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this Keyman content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l Keyman <example-Keyman-file>
    ...
    # Dev Note:
    # This is where you process the Keyman content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-Keyman-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for Keyman content, you may
    #   want to use it.

# Registers the Keyman language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koKeymanLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language Keyman")
    registry.registerLanguage(KoKeymanLanguage())

class KoKeymanLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "Keyman"
    lexresLangName = "Keyman"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "1f967b10-2c7e-4d03-9cd7-5cfc26226dd1"
    defaultExtension = '.kmn'
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'Keyman'}
